...
export default function ProductCard(product) {
  ...
  const imageUrl = product.product.images && product.product.images[0]?.asset?.url
   ...
   const truncatedName = product.product.name.length > 50 
   ? product.product.name.slice(0, 50) + "..." 
   : product.product.name;
  return (
    <Link href={`../ProductPage/${product.product.slug.current}`} className="bg-gray-200 rounded-xl cursor-pointer hover:scale-[1.03] transition-all relative overflow-hidden">
      <div className="">
        <div className="w-full h-[220px] overflow-hidden mx-auto aspect-w-16 aspect-h-8">
          <Image
            ...
            alt={product.product.name} // Dynamically set the alt text based on the product name
            width={1000}
            height={1000}
      ...
      <div className="text-center bg-gray-100 px-2 py-6">
        ...
        <h4 className="text-lg text-gray-800 font-bold mt-6">
          DA{product.product.discountedprice} <strike className="text-red-400 ml-2 font-medium">DA{product.product.price}</strike>
        </h4>
